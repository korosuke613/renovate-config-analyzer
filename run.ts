...
const main = async () => {
    ...
    if(process.env.RENOVATE_TOKEN === undefined) {
      throw new Error("RENOVATE_TOKEN is not set, ex: RENOVATE_TOKEN=$(gh auth token)")
    }
    ...
    const originalEnsureDependencyDashboard = dependencyDashboardHelper.ensureDependencyDashboard;
    ...
    spy4.mockImplementation(async (branchName) => {
      return false;
    });
